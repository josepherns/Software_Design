...
class Suppliers(db.Model):
    ...
    barcode = db.Column(db.Integer, db.ForeignKey(Products.barcode))
    contactnumber = db.Column(db.Integer)
    officeaddress = db.Column(db.String(50))

    def __init__(self,supplierid,suppliername, barcode, contactnumber, officeaddress):
...
class SuppliersMeta(ma.Schema):
    class Meta:
        fields = ("supplierid","suppliername","barcode","contactnumber", "officeaddress")

supplier_meta = SuppliersMeta()
suppliers_meta = SuppliersMeta(many = True)

#NOT DONE , HINDI PA TAPOS MGA COLUMNS
...
@app.route('/supplier_form', methods=['GET','POST'])
def supplier_form():
    if not session.get("name"):
        return redirect("/")
    else:
        if request.method == 'POST':
            SupplierID2=request.form.get("SupplierID_Input")
            SupplierName=request.form.get("SupplierName_Input")
            ProductID=request.form.get("barcodee_Input")
            ContactNumber=request.form.get("ContactNumber_Input")
            OfficeAddress=request.form.get("OfficeAddress_input")
            Check_SupplierID= Suppliers.query.filter_by(SupplierID=SupplierID2).first()
            if(Check_SupplierID is None):
                supplierss = Suppliers(SupplierID2, SupplierName, barcodee_Input, ContactNumber, OfficeAddress)
                msg="Successfully_Added"
                db.session.add(supplierss)
                db.session.commit()
                return redirect(url_for('supplier',msg=msg))
            else:
                msg="Existing_SupplierID"
                return redirect(url_for('supplier',msg=msg))
...
def product_form():
    if not session.get("name"):
        ...
        if request.method == 'POST':
            ...
            Check_Barcode= Products.query.filter_by(barcode=barcode).first()
